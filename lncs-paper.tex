...
\keywords{Datatype-generic programming \and Sums of products \and Recursion \and Overlapping instances \and Closed type families \and Zippers \and Mutually recursive datatypes \and Haskell.}
\end{abstract}
%
%
%
...
\texttt{
\vs
\indent injections \ann \fa xs f. SListI xs \Ra NP (Injection f xs) xs
\vs
\indent newtype Injection f xs a = Inj \{apInj \ann f a \ra K (NS f xs) a\}
\vs
}
For \texttt{f} instantiated to \texttt{NP I} and \texttt{xs} to be the sum of products reflecting a representation code, \texttt{injections} creates a product of all constructor choices that inject appropriate constructor arguments into each sum.

We now can witness the choice using the proof of type equality we have defined in Section~\ref{sec:handling-recursion}. In the following definition, \texttt{f} is supposed to be instantiated to \texttt{Injection yss} where \texttt{yss} reflects the code of the context\footnote{This might be simplified by using a singleton type for \texttt{ConsN} instead of defining a type class function. However, when using a singleton, the recursive definition of \texttt{toFirstCtx} we give further requires a constraint on its function signature that leads to a nonterminating computation.}:
\texttt{
\vs
\indent class\ind Proof (Equal a b) (f a) (f b) \Ra ProofF f a b\\
\indent instance Proof (Equal a b) (f a) (f b) \Ra ProofF f a b
\vs
\indent class ConsNInj (n \ann ConsN) (xs \ann [*]) where\\
\indent\s\s consNInj \ann All (ProofF f xs) yss \Ra NP f yss \ra f xs
\vs
\indent instance ConsNInj n xs \Ra ConsNInj ('N n) xs where\\
\indent\s\s consNInj (\_ :* yss) = consNInj @n yss\\
\indent\s\s consNInj Nil\ind\s\s\s\hs \hs\ths\tths= impossible\\
\indent instance ConsNInj 'F xs where\\
\indent\s\s consNInj ((ys \ann f ys) :* \_)\\
\indent\ind = fromMaybe impossible \$ witnessEq @(Equal xs ys) ys\\
...
As long as the constructor index and product are computed correctly, the proof should never fail (\texttt{impossible}).

Finally, we define a function that applies the injection and returns the constructed context for the given product, representing its constructor arguments, and index of that constructor:
\texttt{
\vs
\indent type AppInj n xs code = (ConsNInj n xs, SListI code,\\
\indent\ind\ind\ind\ind\ind\ind\s All (ProofF (Injection code) xs) code)
\vs
\indent appInjCtx \ann \fa a n xs fam. AppInj n xs (CtxCode fam a)\\
...
The definition of \texttt{toFirstCtx} can now be given using \texttt{appInjCtx}. The following uses a type-level function \texttt{FstRecToHole} to compute the product type by replacing the first recursive occurence in the given datatype's product list with type \texttt{Hole}. Its definition is straightforward, and is omitted here.
\texttt{
\vs
...
\indent\s\s = appInjCtx @a @(CtxConsN (CtxCode fam a) n) \$\\
...
We have established the mechanism of translation, which is cumbersome but verifying, via the proof, that the constructed context will be correct for any given datatype and family. The other functions in the zipper interface can be implemented according to this technique.
...
\indent \ind\ind\ind\ind\s (Injection (CtxCode fam a))\\
