...
The following exploits the idea of \emph{pattern matching}. As before, we consider two cases. In the first case, when the position we are matching on is not recursive, we only require it to be an  instance of \texttt{Show}, and invoke its \texttt{show} function. Whereas in the case of recursion point, we surround it with parentheses and apply our generic function \texttt{gshow}. Thus, the type family for equality model a form of pattern matching on the types again:
\texttt{
\vs
